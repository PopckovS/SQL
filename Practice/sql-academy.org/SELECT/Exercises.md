...
**Задача:**
Получить возраст, самого молодого студента.

**Условие:**
Вывести год `year`

**Решение:**
`TIMESTAMPDIFF` - возвращает значение после вычитания выражения `datetime`
из другого `datetime`.

`1`-й параметр указывает что вернуть из вычитания даты из даты, `2` и `3` аргументы
это даты для сравнения, вычитая одну дату из другой, находим разницу, и получаем
разницу всех дат с текущим числом, ответ :
...
Таким образом получаем массив из разницы дат, и далее отсеиваем из него 
самое минимальное значение функцией `MIN()`   
