...
`AS {smallint | int | bigint}` - при создании последовательности мы можем указывать
ее тип, то есть размер хранимого числа, на выбор есть 3 типа `smallint`, `int`, 
`bigint` по дефолту если тип не указан то используется `bigint`.

---

`START <number>` - указывает с какого числа стартовать последовательность, по дефолту
это число 1.

---

`INCREMENT <number>` - указывает приращение(число для инкрементации) последовательности,
то число на которое последовательность будет увеличиваться с каждым обращением к ней.
...
---
Соглашение о названии `tablename_columnname_seq`
---

Когда мы создаем последовательность для использования с первичным ключом таблицы,
следует называть ее в соответствии с соглашением, когда указывается название
таблицы, столбца и ключевого слова `seq` через нижнее подчеркивание, получаем 
следующий шаблон `tablename_columnname_seq`.

Когда используется ярлык `serial` или `bigserial` то последовательность создается
в соответствии с этим шаблоном.

---
Последовательности и первичный ключ
---

При создании таблиц в `postgresql` для автоинкриментации первичного ключа используются
последовательности, для того чтобы не писать всю команду создания последовательности
и связывать ее с первичным ключом, был создан специальный ярлык, короткая команда,
это `serial` и `bigserial`, которые создают последовательность соответствующего типа
и связывают ее с первичным ключом.

```sql
-- Пример создания таблицы
CREATE TABLE public.report (
	id serial NOT NULL, -- bigserial указывается сразу после названия поля 
);
```

Использование `serial` эквивалентно применению следующей команды, с начала создается 
последовательность в соответствии с шаблоном наименования, после создается сама 
таблица, для столбца `id` указывается дефолтное значение связанное с 
последовательностью, далее последовательность связывается со столбцом таблицы, что бы 
при удалении столбца удалялась и последовательность.

Ограничение `NOT NULL` используется, чтобы гарантировать чтобы значение было 
обязательно.

```sql
CREATE SEQUENCE report_id_seq AS integer;
CREATE TABLE report (
